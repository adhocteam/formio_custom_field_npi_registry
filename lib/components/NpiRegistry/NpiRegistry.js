...
var NpiRegistry = /** @class */ (function (_super) {
    ...
    NpiRegistry.prototype.showList = function (data) {
        ...
        Array.from(ths).forEach(function (th, i) {
            _this.addEventListener(th, 'click', function () { return _this.sortTable(i); });
        });
        ...
        data.results.forEach(function (_a) {
            ...
            var newTr = "<tr><td>" + basic.first_name + "</td><td>" + basic.last_name + "</td><td>" + number + "</td><td>" + addresses[0].address_1 + "</td><td>" + addresses[0].city + "</td><td>" + addresses[0].state + "</td><td>" + addresses[0].postal_code.substring(0, 5) + "</td></tr>";
            ...
            _this.addEventListener(newRow, 'click', function (e) {
                ...
                Array.from(trs).map(function (tr) { return tr.classList.remove('table-warning'); });
                e.currentTarget.classList.add('table-warning');
                _this.selectedNumber = number;
                _this.updateValue();
            });
        });
    };
    NpiRegistry.prototype.sortTable = function (n) {
        var rows;
        var switching;
        var i;
        var x;
